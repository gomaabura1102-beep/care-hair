...
通常の商品画像は使用していません。購入・アフィリエイトリンクを追加するときは、`data/products.ts` の対象商品にある `affiliateUrl` へURLを設定してください。提携先から正式に提供された商品画像がある場合だけ、同じ商品へ `affiliateImageUrl` も設定できます。`affiliateUrl` が空文字の間は購入ボタンもアフィリエイト画像も表示されません。
